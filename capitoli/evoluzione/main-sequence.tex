...
\begin{figure}
    \centering
    \includegraphics[width = 0.4\textwidth]{immagini/ZAMS.png}
    \caption{La figura mostra la Zero-Age Main Sequence in azzurro e la traccia di Hayashi in rosso.}\label{fig:ZAMS}
\end{figure}
...
\[
    \epsilon_\textup{pp} \propto \rho X^2 T^5
\]
\[
    \epsilon_\textup{CNO} \propto \rho X X_\textup{CNO} T^{15}
\]

Per cui si ha che, per masse superiori a $1.2 \si{\solarmass}$, $\epsilon_\textup{pp} \ll \epsilon_\textup{CNO}$ se la temperatura aumenta, lo farà anche la produzione di energia, assieme alla luminosità. Da quanto visto precedentemente l'aumento di luminosità implica un flusso più intenso. Diventa quindi necessario dissipare questa energia più velocemente, attivando i processi convettivi. Se infatti il flusso aumenta allora aumenta anche il gradiente radiativo e nel momento in cui $\nabla_\textup{rad} > \nabla_\textup{ad}$ si attiverà la convezione nel nucleo.
...
\[
    t_\textup{MS} \simeq 10^{10} M^{-3} (\si{yr})
\]
...
\begin{table}
    ...
    \begin{tabular}{c|ccccccc}
        ...
        \midrule
        $t_\textup{MS}$ [$\si{yr}$] & $1 \times 10^7$ & $ 2.2 \times 10^7$ & $7 \times 10^7$ & $2\times 10^8$ & $5\times 10^8$ & $1.7 \times 10^9$ & $9\times 10^9$\\
        \bottomrule
    \end{tabular}
\end{table}

Si osserva che data una stella di circa $0.8 \si{\solarmass}$, la durata della sua MS ha lo stesso ordine di grandezza dell'età dell'universo ($t_\textup{MS} \sim 13.9\times 10^9 \, \textup{yr}$). Questo vuol dire che stelle con massa inferiore ad essa e nate poco tempo dopo l'origine dell'universo, saranno ancora nel pieno della propria main sequence.